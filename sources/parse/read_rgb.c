...
/*   Updated: 2026/02/25 11:43:31 by adeimlin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
uint32_t	ft_strtorgb(const char *str, const char **str_ptr)
{
	size_t		i;
	uint32_t	rgb_value;
	int64_t		number;
	...
	while (i < 4)
	{
		...
		if (number < 0 || number > 255 || (*str++ != ',' && i < 3))	// Optionally could just cast to char and read any value
		{
			if (str_ptr != NULL)
				*str_ptr = NULL;
			return (0);
		}
		rgb_value = (rgb_value << 8) + (uint32_t) number;
		i++;
	}
	if (str_ptr != NULL)
		*str_ptr = str;
